...
class AnomalyDetector:
    ...
    def detect_anomalies(self, metrics_data):
        ...
        try:
            ...
            missing_cols = [col for col in self.model_trainer.feature_columns if col not in metrics_df.columns]
            if missing_cols:
                missing_df = pd.DataFrame({col: [0] for col in missing_cols}, index=metrics_df.index)
                metrics_df = pd.concat([metrics_df, missing_df], axis=1)
